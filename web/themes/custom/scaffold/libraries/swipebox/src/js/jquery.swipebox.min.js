...
+!function (a, b, c, d) {
    c.swipebox = function (e, f) {
        var g, h, i = {
            ...
            toggleClassOnLoad: "",
            selector: null
        }, j = this, k = [], l = navigator.userAgent.match(/(iPad)|(iPhone)|(iPod)|(Android)|(PlayBook)|(BB10)|(BlackBerry)|(Opera Mini)|(IEMobile)|(webOS)|(MeeGo)/i), m = null !== l || b.createTouch !== d || "ontouchstart"in a || "onmsgesturechange"in a || navigator.msMaxTouchPoints, n = !!b.createElementNS && !!b.createElementNS("http://www.w3.org/2000/svg", "svg").createSVGRect, o = a.innerWidth ? a.innerWidth : c(a).width(), p = a.innerHeight ? a.innerHeight : c(a).height(), q = 0;
        j.settings = {}, c.swipebox.close = function () {
            g.closeSlide()
        }, c.swipebox.extend = function () {
            return g
        }, j.init = function () {
            j.settings = c.extend({}, i, f), c.isArray(e) ? (k = e, g.target = c(a), g.init(j.settings.initialIndexOnArray)) : c(e).on("click", j.settings.selector, function (a) {
                if ("slide current" === a.target.parentNode.className)return !1;
                g.destroy(), h = null === j.settings.selector ? c(e) : c(e).find(j.settings.selector), k = [];
                var b, d, f;
                f || (d = "data-rel", f = c(this).attr(d)), f || (d = "rel", f = c(this).attr(d)), f && "" !== f && "nofollow" !== f && (h = h.filter("[" + d + '="' + f + '"]')), h.each(function () {
                    var a = null, b = null;
            ...
            init: function (a) {
                ...
                var a, b, d, e, f, g, h = this, i = !1, j = !1, l = 10, m = 50, n = {}, p = {}, r = c("#swipebox-top-bar, #swipebox-bottom-bar"), s = c("#swipebox-slider");
                r.addClass("visible-bars"), h.setTimeout(), c("body").bind("touchstart", function (h) {
                    return c(this).addClass("touching"), a = c("#swipebox-slider .slide").index(c("#swipebox-slider .slide.current")), p = h.originalEvent.targetTouches[0], n.pageX = h.originalEvent.targetTouches[0].pageX, n.pageY = h.originalEvent.targetTouches[0].pageY, c("#swipebox-slider").css({
                        "-webkit-transform": "translate3d(" + q + "%, 0, 0)",
                        transform: "translate3d(" + q + "%, 0, 0)"
                    }), c(".touching").bind("touchmove", function (h) {
                        if (h.preventDefault(), h.stopPropagation(), p = h.originalEvent.targetTouches[0], !j && (f = d, d = p.pageY - n.pageY, Math.abs(d) >= m || i)) {
                            var r = .75 - Math.abs(d) / s.height();
                            s.css({top: d + "px"}), s.css({opacity: r}), i = !0
                        }
                        e = b, b = p.pageX - n.pageX, g = 100 * b / o, !j && !i && Math.abs(b) >= l && (c("#swipebox-slider").css({
                            "-webkit-transition": "",
                            transition: ""
                            ...
                            "-webkit-transform": "translate3d(" + (q + g) + "%, 0, 0)",
                    ...
                    if (a.preventDefault(), a.stopPropagation(), c("#swipebox-slider").css({
                        ...
                        }), d = p.pageY - n.pageY, b = p.pageX - n.pageX, g = 100 * b / o, i)if (i = !1, Math.abs(d) >= 2 * m && Math.abs(d) > Math.abs(f)) {
                        ...
                        top: 0,
                        opacity: 1
                    }, 300); else j ? (j = !1, b >= l && b >= e ? h.getPrev() : b <= -l && b <= e && h.getNext()) : r.hasClass("visible-bars") ? (h.clearTimeout(), h.hideBars()) : (h.showBars(), h.setTimeout());
                ...
                q = 100 * -a, this.doCssTrans() ? d.css({
                    "-webkit-transform": "translate3d(" + 100 * -a + "%, 0, 0)",
                    transform: "translate3d(" + 100 * -a + "%, 0, 0)"
                }) : d.animate({left: 100 * -a + "%"}), c("#swipebox-slider .slide").removeClass("current"), c("#swipebox-slider .slide").eq(a).addClass("current"), this.setTitle(a), b && d.fadeIn(), c("#swipebox-prev, #swipebox-next").removeClass("disabled"), 0 === a ? c("#swipebox-prev").addClass("disabled") : a === k.length - 1 && !0 !== j.settings.loopAtEnd && c("#swipebox-next").addClass("disabled")
            }, openSlide: function (b) {
                c("html").addClass("swipebox-html"), m ? (c("html").addClass("swipebox-touch"), j.settings.hideCloseButtonOnMobile && c("html").addClass("swipebox-no-close-button")) : c("html").addClass("swipebox-no-touch"), c(a).trigger("resize"), this.setSlide(b, !0)
                ...
                return c || d ? (d && (c = d), f = g.parseUri(a, {
                    ...
                    v: ""
                }), b = '<iframe width="560" height="315" src="//' + c[1] + "/embed/" + c[2] + "?" + f + '&rel=0" frameborder="0" allowfullscreen></iframe>') : e ? (f = g.parseUri(a, {
                    autoplay: j.settings.autoplayVideos ? "1" : "0",
                    byline: "0",
                    portrait: "0",
